...
import { useTranslation } from 'react-i18next'; // חיבור לi18n 
...
const TransactionsPage = () => {
  ...
  return (
    <div className="p-4 transactions-page">
        ...
        <h1>{t('transactions.dashboardTitle')}</h1>
      {/* כפתור לפתיחת מודל */}
      <button
        className="add-btn"
        onClick={() => {
      ...
      <section className="grid kpis">
        <KpiCard
          // title="סה'כ הכנסות"
          title={t('transactions.kpi.totalIncome')}
          value={formatCurrency(totals.income)}
          icon={<Wallet />}
          // caption="במצטבר לחצי שנה"
          caption={t('transactions.kpi.halfYearAccum')}
        />
        <KpiCard
        //  title="סה'כ הוצאות"
        title={t('transactions.kpi.totalExpense')}
        ...
        <KpiCard
          // title="סה'כ חסכונות"
          title={t('transactions.kpi.totalSavings')}
          value={formatCurrency(totals.income - totals.expense)}
          icon={<PiggyBank />}
          // caption="הכנסות פחות הוצאות"
          caption={t('transactions.kpi.savingsCaption')}
        />
      </section>
      ...
      <h2>{t('transactions.chartsTitle')}</h2>
      <div>
